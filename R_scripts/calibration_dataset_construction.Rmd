...
Calibration datasets including a range of random and guided samples from the baseline activity profile are created. The number of nodes corresponding to 10%, 20%, ..., 100% of the total number of network nodes is calculated.
...
The Determinative Power score for all network nodes is calculated using functions provided by Weidner et al (source). Scores are added to the network dataframe as a new column.
...
The degree Z-score for all network nodes is calculated using functions provided by Weidner et al (source). Scores are added to the network dataframe as a new column.
...
```{r}
bash_script_random(20, percentages)
